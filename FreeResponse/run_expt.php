		...
		if ($posted_filetag == $filetag) {
			...
			foreach ($_POST as $key => $value) {
				if ($key != 'mossubmitter' && $key != 'posted_filetag') {
					...
					$query = "SELECT * FROM answers WHERE filetag = '" . $filetag . "' AND qv_no = '" . $qv_no . "';";
					$answer = ask_pg($db_credentials,$query);
					// format the string correctly for the datbase
					$answer = pg_fetch_assoc($answer);
					// submit the current response to the database
					$query = "INSERT INTO votes(user_number,qv_no, user_response, answer, location, ses, src, vi, ai) VALUES('" . $user_number . "', '" . $qv_no . "', '" . $response . "', '" . $answer['answer'] . "', '" . $location . "', '" . $session . "', '" . $prev_src . "', '" . $prev_vi . "', '" . $prev_ai ."');";
...
foreach ($q_list as $qkey => $qvalue) {
	if ($qkey != 0) {
		echo "<h1>" . $qvalue . "</h1>\r";
		echo "<p><input type=\"text\" id=\"text_" . $qkey . "\" name=\"" . $qkey . "\"></p>\r";
	}
}
