...
if (isset($_POST['db_submitter'])){
	...
	switch ($request){
		...
		// one for each viewo
		case "by video":
			$query = "SELECT src, vi, ai FROM votes GROUP BY src, vi, ai ORDER BY src;";
			$result = ask_pg($db_credentials,$query);
			$title = "<p>Answers organized by video and position:</p>";
			$case = "vidbypos";
			$explanation = "<p>This shows the answers organized by video.</p>";
			break;
	   ...
	   switch($selected_radio){
	   	case "htmlout":
	   		...
	   		if($case == "vidbypos") {
	   			...
	   			// results has the list of file tags for the videos. for each video, a seperate
	   			...
	   			while ($myrow = pg_fetch_assoc($result)) {
					...
					$query = "SELECT correctness, answer FROM votes WHERE src = '" . $myrow['src'] . "' AND vi = '" . $myrow['vi'] . "' AND ai = '" . $myrow['ai'] . "' ORDER BY answer;";
...
<option value="by video">organize votes by videos</option>
